# test02_packer/terraform/compute.tf
...
data "aws_ami" "latest_web" {
  most_recent = true
  # 직접만들었으니 owners가 amazon에서 self 가 된다
  owners      = ["self"]
  filter {
    name   = "name"
    # packer 에서 만든 이름을 참조할수있도록 한다
    # packer 에서 ami name을 ami-web-{{timestamp}} 로 했음
    values = ["ami-web-*"]
  }
}
...
resource "aws_launch_template" "lt" {
  ...
  # ami 혹은 user_data가 변경이 되었을때 default 버전을 변경하도록 ㅎ낟 
...
resource "aws_autoscaling_group" "asg" {
  ...
  launch_template {
    ...
    # launche template 의 소스코드가 바뀌어서 버전번호가 올라가면 바로 감지 할 수 있도록 한다
  ...
  instance_refresh {
    ...
    strategy = "Rolling"
    preferences {
      # 교체중에 최소한 몇%의 인스턴스가 살아있어야 하는지 
      min_healthy_percentage = 50
      # 새 인스턴스가 뜨고 나서 다음 인스턴스를 교체하기전까지의 시간
